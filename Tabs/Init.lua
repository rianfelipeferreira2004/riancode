...
function TabsManager:SelectTab(SelectedTab, SelectedPage)
    for _, data in ipairs(self.Tabs) do
        ...
        local Badge = data.Tab:FindFirstChild("TabBadge")
        local BadgeText = Badge and Badge:FindFirstChild("TabBadgeText")
        ...
        if Badge then
            TweenService:Create(Badge, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(38, 40, 54)}):Play()
        end
        if BadgeText then
            TweenService:Create(BadgeText, TweenInfo.new(0.15), {TextColor3 = Color3.fromRGB(155, 155, 175)}):Play()
        end
    end
    ...
    TweenService:Create(SelectedTab, TweenInfo.new(0.15), {BackgroundTransparency = 0.82}):Play()
    local Indicator = SelectedTab:FindFirstChild("Indicator")
    local TabText = SelectedTab:FindFirstChild("TabText")
    local Badge = SelectedTab:FindFirstChild("TabBadge")
    local BadgeText = Badge and Badge:FindFirstChild("TabBadgeText")
    if Indicator then
    ...
    if Badge then
        TweenService:Create(Badge, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(105, 90, 190)}):Play()
    end
    if BadgeText then
        TweenService:Create(BadgeText, TweenInfo.new(0.15), {TextColor3 = Color3.fromRGB(255, 255, 255)}):Play()
    end
